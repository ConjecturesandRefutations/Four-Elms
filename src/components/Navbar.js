...
<nav class="text-right xl:fixed" >
    ...
    <ul class={`text-sm ${menuOpen ? "" : "hidden"} inline-block md:block mt-4`} id="menu">
        ...
        <li class="text-gray-700 py-2">
            <Link to={'/check-in-out'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span> {lang==='english' ? 'Check-in and Check-out Procedures' : 'Procedimientos de registro y salida'}</span>
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/contact'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span> {lang==='english' ? 'Contact and emergency information' : 'Información de contacto y emergencia'}</span>            
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/alarm'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
        ...
        <li class="py-2">
            <Link to={'/towels'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span>{lang==='english' ? 'Towels, linen & kitchen supplies' : 'Toallas, ropa de cama y utensilios de cocina'}</span>               
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/waste'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span>{lang==='english' ? 'Waste disposal and recycling' : 'Eliminación y reciclaje de residuos'}</span>                      
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/information'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span>{lang==='english' ? 'Information on the local area' : 'Información sobre el área local'}</span>               
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/contract'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span>{lang==='english' ? 'House rules & contract' : 'Reglas de la casa y contrato'}</span>              
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/cleaning'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
                <span>{lang==='english' ? 'House cleaning' : 'Limpieza de la casa'}</span>             
            </Link>
        </li>
        <li class="py-2">
            <Link to={'/feedback'} class="px-4 flex justify-end mt-4" onClick={handleMenuClick}>
